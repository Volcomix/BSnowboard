...
uniform vec4 unf19;
const float cons21 = float(1.000000);
const vec4 cons25 = vec4(1.000000, 1.000000, 1.000000, 0.005000);
const vec4 cons31 = vec4(0.800000, 0.800000, 0.800000, 0.000000);
const float cons36 = float(50.000000);
const float cons39 = float(0.500000);
uniform vec3 unf44;
const vec3 cons45 = vec3(1.000000, 1.000000, 1.000000);
const float cons58 = float(1.000000);
...
void main(void)
{
	...
	set_rgb_zero(tmp5);
	set_rgb_zero(tmp6);
	lamp_visibility_sun_hemi(unf7, tmp8, tmp9, tmp10);
	shade_inp(tmp2, tmp8, tmp13);
	shade_visifac(tmp13, tmp10, cons16, tmp17);
	shade_mul_value(tmp17, unf19, tmp20);
	mtex_value_invert(cons21, tmp22);
	mix_mult(tmp22, tmp20, cons25, tmp26);
	mtex_value_invert(tmp22, tmp28);
	shade_madd(vec4(tmp5, 1.0), tmp26, cons31, tmp32);
	shade_cooktorr_spec(tmp2, tmp8, tmp4, cons36, tmp37);
	shade_spec_t(tmp28, cons39, tmp10, tmp37, tmp42);
	shade_add_spec(tmp42, unf44, cons45, tmp46);
	shade_add_clamped(vec4(tmp6, 1.0), vec4(tmp46, 1.0), tmp49);
	shade_clamp_positive(tmp49, tmp51);
	shade_clamp_positive(tmp32, tmp53);
	shade_add(tmp53, tmp51, tmp56);
	mtex_alpha_to_col(tmp56, cons58, tmp59);
